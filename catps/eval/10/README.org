...
2. As respostas são dadas em notação pré e pósfixada:
   ...
   Explicação para o item 2.

   Criamos a árvore seguindo a associação a esquerda, da seguinte
   forma: =((((A * B) * C) * D) * E)=. Obtemos a seguinte árvore:
   ...
   =visitar o filho da direita=. No exemplo acima (item 2), para obter a
   notação pré-fixada, começando sempre pelo nós raiz, temos:

   =* * * * A B C D E=

   Para obter a notação pós-fixada teremos:
